...
WingExtremes.RootTE = WingExtremes.RootLE + (YRot*[config.RootChord(iBody) 0 0]')';
% WingExtremes.TipLE = [0 config.SemiSpan(iCorpo)*cosd(config.DihedralAngle(iCorpo)) 0];
WingExtremes.TipLE = [0 config.SemiSpan(iBody) 0];
% Adding Sweep angle
WingExtremes.TipLE(1) = WingExtremes.RootLE(1) + config.RootChord(iBody)/4 + config.SemiSpan(iBody) * tand(config.SweepAngle(iBody)) - config.TipChord(iBody)/4;
...
WingExtremes.TipTE = WingExtremes.TipLE +  (YRot*[config.TipChord(iBody) 0 0]')';

WingExtremes.RootLE = WingExtremes.RootLE + [config.LEPosition_X(iBody) config.LEPosition_Y(iBody) config.LEPosition_Z(iBody)];
WingExtremes.RootTE = WingExtremes.RootTE + [config.LEPosition_X(iBody) config.LEPosition_Y(iBody) config.LEPosition_Z(iBody)];
WingExtremes.TipLE = WingExtremes.TipLE + [config.LEPosition_X(iBody) config.LEPosition_Y(iBody) config.LEPosition_Z(iBody)];
WingExtremes.TipTE = WingExtremes.TipTE + [config.LEPosition_X(iBody) config.LEPosition_Y(iBody) config.LEPosition_Z(iBody)];
...
TEDiscr = linspace(0, 1, config.SemiSpanwiseDiscr(iBody)+1);
...
TipDiscr = linspace(0, 1, config.ChordwiseDiscr(iBody)+1);
...
internalMesh = cell( config.ChordwiseDiscr(iBody), config.SemiSpanwiseDiscr(iBody)*2 );
Vortices = cell( config.ChordwiseDiscr(iBody), config.SemiSpanwiseDiscr(iBody)*2 );
ControlPoints = cell( config.ChordwiseDiscr(iBody), config.SemiSpanwiseDiscr(iBody)*2 );
InducedPoints = cell( config.ChordwiseDiscr(iBody), config.SemiSpanwiseDiscr(iBody)*2 );
Normals = cell( config.ChordwiseDiscr(iBody), config.SemiSpanwiseDiscr(iBody)*2 );

InfiniteVortices = cell( config.ChordwiseDiscr(iBody), config.SemiSpanwiseDiscr(iBody)*2 );

InfiniteVorticesLength = 50 * config.RootChord(iBody);
...
yShift = config.LEPosition_Y(iBody);
